...
#define MEM_ALIGN (1 << (MEM_ALIGN_SHIFT))
...
static inline void push_slab_head(void* mem) {
  ...
  int mem_slabs_index = size >> MEM_ALIGN_SHIFT;
...
static inline void* pop_slab_head(size_t size) {
  ...
  fast_mem_node_t* mem_node = mem_slabs[size >> MEM_ALIGN_SHIFT].next;

  if (mem_node) {
    mem_slabs[size >> MEM_ALIGN_SHIFT].next = mem_node->next;
    mem_node->next = NULL;
    result = mem_node;
...
__attribute__((always_inline)) void* fast_malloc(size_t requested_size) {
  ...
  result_mem = pop_slab_head(requested_size);
...
__attribute__((always_inline)) void fast_free(void* mem) {
  push_slab_head(mem);
}
