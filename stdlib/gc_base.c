...
static inline gc_scope_stack_el_t* get_nearest_func_scope() {
  ...
  return &(gc_scope_stack
               .stack[gc_scope_stack.curr_index - top->func_scope_offset]);
}
...
/* gc_free
 *
 * Frees object and any references to the object as well as removing it from the
 *global linked
...
void gc_push_func_scope(uint64_t num_named_vars_in_scope) {
  ...
  if (num_named_vars_in_scope > 0) {
    scope_el->named_vars =
        fast_zalloc(sizeof(gc_base_t) * num_named_vars_in_scope);
  } else {
...
void gc_pop_scope(void) {
  ...
  GC_ASSERT(gc_stack.curr_index >= 0 && "Object stack undeflow!");
